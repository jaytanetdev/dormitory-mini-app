  ...
  return <>
    <div className="page">
      ...
      <section aria-label="ข้อมูลห้องและยอดปัจจุบัน">
        ...
        <div className="card balance-card"><div className="balance-row"><div><p className="eyebrow">ยอดที่ต้องชำระ</p><p className="amount">฿{formatBaht(data.invoice.total)}</p><p className="muted small">ภายใน {formatThaiDate(data.invoice.dueAt)}</p></div><StatusPill status={data.invoice.status} /></div><Link href={`/pay/${data.invoice.id}`} className="primary-button full-width" style={{ marginTop: 16 }}><Banknote size={19} />ชำระบิลนี้</Link></div>
      </section>
